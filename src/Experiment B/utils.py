...
class Net1(nn.Module):
	def __init__(self, tau, dt, input_size, output_size, hidden_size, g_G, seed, h0):
		...
		self.I = nn.Parameter(torch.Tensor(hidden_size, input_size), requires_grad=False)
...
def pca_trials(net, plot=False, return_dynamics = False):
	...
	if plot == True:
		...
		plt.show()
		plt.close()
	if return_dynamics == False:	
		return e_val, e_vec
	else:
		return e_val, e_vec, r_time1, r_time2, r_time3

def I_partic_ratio(net, Nt, O_on, SO_on, O_off, SO_off, thresh, perc, cost_onset, sig):
	"""Participation ration of weight vector I and random network PCA eigenvectors"""
	I = net.I.detach().numpy().flatten()
	with torch.no_grad():
	...
	part_ratio = np.square(np.sum(np.absolute(components)))/np.sum(np.square(components))
...
def w_partic_ratio(net, Nt, O_on, SO_on, O_off, SO_off, thresh, perc, cost_onset, sig):
	...
	return part_ratio
def pc_trajectories(net, Nt, O_on, SO_on, O_off, SO_off, thresh, perc, cost_onset, sig):
	I = net.I.detach().numpy().flatten()
	w = net.w.detach().numpy().flatten()	
	#PCA on random network
	with torch.no_grad():
		net.h0 = torch.nn.Parameter(.1 * torch.ones_like(net.h0), requires_grad=False)
	r_time0, output0 = signals(net, 0, 10000, 0)
	e_val0, e_vec0 = pca_matrix(net, 5000, 10000,r_time0)
	#output
	in1 = 50
	...
	r_time3, output3 = signals(net, O_on, Nt, in3)
	#projecting dynamics
	rproj_time1 = np.empty([Nt,3])
	rproj_time2 = np.empty([Nt,3])
	rproj_time3 = np.empty([Nt,3])
	oproj_time1 = np.empty([Nt,3])
	oproj_time2 = np.empty([Nt,3])
	oproj_time3 = np.empty([Nt,3])
	I_vector = np.empty([net.hidden_size])
	w_vector = np.empty([net.hidden_size])
	for t in range(Nt):
		for i in range(3):
			ax = e_vec0.T[i]
			rproj_time1[t,i] = ax @ r_time1 [t,:]
			rproj_time2[t,i] = ax @ r_time2 [t,:]
			rproj_time3[t,i] = ax @ r_time3 [t,:]
			w_ax = w @ ax
			oproj_time1[t,i] = w_ax * rproj_time1[t,i]
			oproj_time2[t,i] = w_ax * rproj_time2[t,i]
			oproj_time3[t,i] = w_ax * rproj_time3[t,i]
			I_vector[i] = I @ ax
			w_vector[i] = w @ ax
	plt.figure()
	origin = [0,0,0]
	plt.title(f"Firing rates r(t) projected on PC's")
	ax = plt.axes(projection='3d')
	ax.scatter(rproj_time1[:,0], rproj_time1[:,1], rproj_time1[:,2], marker = ".", color = "b", label = f"r(t) {in1}ms")
	ax.scatter(rproj_time2[:,0], rproj_time2[:,1], rproj_time2[:,2], marker = ".", color = "r", label = f"r(t) {in2}ms")
	ax.scatter(rproj_time3[:,0], rproj_time3[:,1], rproj_time3[:,2], marker = ".", color = "g", label = f"r(t) {in3}ms")
	ax.scatter(oproj_time1[:,0], oproj_time1[:,1], oproj_time1[:,2], marker = ".", color = "c", label = f"o(t) {in1}ms")
	ax.scatter(oproj_time2[:,0], oproj_time2[:,1], oproj_time2[:,2], marker = ".", color = "m", label = f"o(t) {in2}ms")
	ax.scatter(oproj_time3[:,0], oproj_time3[:,1], oproj_time3[:,2], marker = ".", color = "y", label = f"o(t) {in3}ms")
	ax.quiver(origin[0], origin[1], origin[2], I_vector[0], I_vector[1], I_vector[2], color = "black", label = "I")
	ax.quiver(origin[0], origin[1], origin[2], w_vector[0], w_vector[1], w_vector[2], color = "grey", label = "w")
	ax.set_xlabel("PC1")
	ax.set_ylabel("PC2")
	ax.set_zlabel("PC3")
	plt.legend()
...
def pc_trajectories_threshold(net, Nt, O_on, SO_on, O_off, SO_off, thresh, perc, cost_onset, sig):
	I = net.I.detach().numpy().flatten()
	w = net.w.detach().numpy().flatten()	
	#PCA on threshold dynamics
	e_val, e_vec, r_time1, r_time2, r_time3 = pca_trials(net, plot=True, return_dynamics = True)
	#projecting dynamics
	Nt = np.shape(r_time1)[0]
	rproj_time1 = np.empty([Nt,3])
	rproj_time2 = np.empty([Nt,3])
	rproj_time3 = np.empty([Nt,3])
	oproj_time1 = np.empty([Nt,3])
	...
	plt.figure()
	origin = [0,0,0]
	plt.title(f"Firing rates r(t) projected on PC's of threshold dynamics")
	ax = plt.axes(projection='3d')
	ax.scatter(rproj_time1[:,0], rproj_time1[:,1], rproj_time1[:,2], marker = ".", color = "b", label = f"r(t) 50ms")
	ax.scatter(rproj_time2[:,0], rproj_time2[:,1], rproj_time2[:,2], marker = ".", color = "r", label = f"r(t) 150ms")
	ax.scatter(rproj_time3[:,0], rproj_time3[:,1], rproj_time3[:,2], marker = ".", color = "g", label = f"r(t) 300ms")
	ax.scatter(oproj_time1[:,0], oproj_time1[:,1], oproj_time1[:,2], marker = ".", color = "c", label = f"o(t) 50ms")
	ax.scatter(oproj_time2[:,0], oproj_time2[:,1], oproj_time2[:,2], marker = ".", color = "m", label = f"o(t) 150ms")
	ax.scatter(oproj_time3[:,0], oproj_time3[:,1], oproj_time3[:,2], marker = ".", color = "y", label = f"o(t) 300ms")
	ax.quiver(origin[0], origin[1], origin[2], I_vector[0], I_vector[1], I_vector[2], color = "black", label = "I")
	ax.quiver(origin[0], origin[1], origin[2], w_vector[0], w_vector[1], w_vector[2], color = "grey", label = "w")
	ax.set_xlabel("PC1")
	ax.set_ylabel("PC2")
